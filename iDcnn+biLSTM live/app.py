...
#  CONFIGURATION & PATHS
# ═══════════════════════════════════════════════════════════════
_MP = r"C:\Users\josep\OneDrive\Desktop\Project data\csi_fall_hybrid_cnnlstm.h5"
...
def _analyse(mdl, data_centered):
    ...
    for i in range(0, len(data_centered) - WIN + 1, STEP):
        ...
        segs.append(norm_win)
        
    if not segs: return None, None, None
    segs_arr = np.array(segs).reshape(-1, WIN, SC, 1)
...
class App:
    ...
    def _run_prediction_classic(self, channel):
        ...
        pred, cf, tag = _analyse(self.mdl, buf)
        sco = self.sc_var.get()
    ...
    def _init_model(self):
        def _go():
            try:
                ...
                m.load_weights(_MP)
                m.predict(np.zeros((1, WIN, SC, 1), dtype=np.float32), verbose=0)
                self.mdl = m
