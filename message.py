...
#  CONFIGURATION & PATHS
# ═══════════════════════════════════════════════════════════════
_MP        = r"C:\Users\josep\OneDrive\Desktop\Project data\MODELS\ResNet1D_20260314_125411.keras"
_SCALER    = r"C:\Users\josep\OneDrive\Desktop\Project data\MODELS\scaler.pkl"
...
# ═══════════════════════════════════════════════════════════════
#  ALERT CONFIGURATION  (WhatsApp via pywhatkit)
# ═══════════════════════════════════════════════════════════════
ALERT_NUMBER = "+919496351723"

def send_fall_alert():
    try:
        pywhatkit.sendwhatmsg_instantly(
            phone_no  = ALERT_NUMBER,
            message   = (
                "🚨 FALL DETECTED!\n"
                "The fall detection system has identified a fall event.\n"
        ...
        print(f"✅  WhatsApp alert sent to {ALERT_NUMBER}")
    ...
    for line in lines:
        if "[" in line and "]" in line:
            try:
                ...
                nums = [int(x) for x in csi.replace(",", " ").split()
                        if x.lstrip("-").isdigit()]
                if len(nums) >= 104:
                    narr = np.array(nums)
...
def _analyse(mdl, data_centered, scaler=None):
    if data_centered is None or len(data_centered) < WIN:
        return None, None, None
    segs = []
    for i in range(0, len(data_centered) - WIN + 1, STEP):
        window = data_centered[i: i + WIN]
    ...
    if mx >= HCT:
        res, tag = 1, f"High confidence  {mx*100:.0f}%"
    elif r >= VR:
        res, tag = 1, f"Voting  {v}/{len(segs)} segments"
    else:
        res, tag = 0, "Below threshold"
...
class App:
    ...
    def _build(self):
        ...
        self.ttl = tk.Label(lf, text="Fall Detection AI",
                            font=("Helvetica", 14, "bold"),
                            fg=T["text"], bg=T["surface"])
        self.ttl.pack(side="left")
        self._r(self.ttl, "fg", "text")
        self._r(self.ttl, "bg", "surface")

        rf = tk.Frame(self.bar, bg=T["surface"])
        rf.place(relx=1, x=-20, rely=0.5, anchor="e")
        self._r(rf, "bg", "surface")

        self.tgl = tk.Label(rf, text="☀", font=("Helvetica", 15),
                            fg=T["sub"], bg=T["surface"], cursor="hand2")
        self.tgl.pack(side="right", padx=(14, 0))
        self.tgl.bind("<Button-1>", lambda e: self._switch_theme())
        self._r(self.tgl, "fg", "sub")
        self._r(self.tgl, "bg", "surface")

        self.st_lbl = tk.Label(rf, text="Initialising…",
                               font=("Helvetica", 9),
                               fg=T["sub"], bg=T["surface"])
        self.st_lbl.pack(side="right", padx=(0, 6))
        self._r(self.st_lbl, "fg", "sub")
        self._r(self.st_lbl, "bg", "surface")

        self.dot_cv = tk.Canvas(rf, width=10, height=10,
    ...
    def _left_col(self):
        ...
        self.sc_disp = tk.Label(c2, text="20",
                                font=("Helvetica", 12, "bold"),
                                fg=self.T["accent"], bg=self.T["surface"])
        self.sc_disp.pack(side="right")
        self._r(self.sc_disp, "fg", "accent")
        self._r(self.sc_disp, "bg", "surface")
        self.sld = tk.Scale(c2, from_=0, to=51, orient="horizontal",
                            variable=self.sc_var, showvalue=False,
                            bg=self.T["surface"], troughcolor=self.T["surface2"],
                            highlightthickness=0, bd=0,
                            command=lambda v: self.sc_disp.configure(
                                text=str(int(float(v)))))
        self.sld.pack(fill="x")
        self._r(self.sld, "bg", "surface")
        self._r(self.sld, "troughcolor", "surface2")

        # ── 1. MANUAL DATA COLLECTION ──
        self._sec("Manual Collection", self.lcol)
        c_col = self._card(self.lcol)

        self.lbl_timer = tk.Label(c_col, text="Ready to Record",
                                  font=("Consolas", 12, "bold"),
                                  fg=self.T["accent"], bg=self.T["surface"])
        self.lbl_timer.pack(pady=(0, 10))
        self._r(self.lbl_timer, "fg", "accent")
        self._r(self.lbl_timer, "bg", "surface")

        btn_fall = tk.Button(c_col, text="🔴 RECORD FALL",
                             bg="#f5505e", fg="white",
                             font=("Arial", 9, "bold"), relief="flat",
                             command=lambda: self.start_collection("fall"))
        btn_fall.pack(fill="x", pady=2)

        btn_safe = tk.Button(c_col, text="🟢 RECORD NO FALL",
                             bg="#26c97a", fg="white",
                             font=("Arial", 9, "bold"), relief="flat",
                             command=lambda: self.start_collection("no fall"))
        btn_safe.pack(fill="x", pady=2)

        # ── 2. CLASSIC SPLIT PREDICTION BUTTON ──
        self._sec("Test Old Data Folders", self.lcol)
        self.btn_cv = tk.Canvas(self.lcol, height=45,
                                bg=self.T["bg"], highlightthickness=0,
                                cursor="hand2")
        ...
        self.lbl_live_status = tk.Label(c_live, text="Ready to simulate & sort",
                                        font=("Arial", 9),
                                        fg=self.T["sub"], bg=self.T["surface"])
        self.lbl_live_status.pack(pady=(0, 6))
        self._r(self.lbl_live_status, "fg", "sub")
        self._r(self.lbl_live_status, "bg", "surface")

        lbl_live = tk.Label(c_live, text="Record live & auto-sort:",
                            font=("Helvetica", 8, "italic"),
                            fg=self.T["sub"], bg=self.T["surface"])
        lbl_live.pack(anchor="w", pady=(0, 4))
        self._r(lbl_live, "fg", "sub")
        self._r(lbl_live, "bg", "surface")
        ...
        lbl_sort = tk.Label(c_live, text="Test file & Save to NEW_ROOM_DATA:",
                            font=("Helvetica", 8, "italic"),
                            fg=self.T["sub"], bg=self.T["surface"])
        lbl_sort.pack(anchor="w", pady=(0, 8))
        self._r(lbl_sort, "fg", "sub")
        self._r(lbl_sort, "bg", "surface")

        self.btn_sort_cv = tk.Canvas(c_live, height=45,
                                     bg=self.T["surface"],
    ...
    def _right_col(self):
        ...
        self.res_lbl = tk.Label(ri, text="Awaiting prediction",
                                font=("Helvetica", 20, "bold"),
                                fg=T["sub"], bg=T["surface"])
        self.res_lbl.pack(anchor="w")
        self._r(self.res_lbl, "bg", "surface")

        self.tag_lbl = tk.Label(ri, text="",
                                font=("Helvetica", 9),
                                fg=T["sub"], bg=T["surface"])
        self.tag_lbl.pack(anchor="w", pady=(3, 0))
        self._r(self.tag_lbl, "fg", "sub")
        self._r(self.tag_lbl, "bg", "surface")

        # WhatsApp Alert Status Label
        self.sms_lbl = tk.Label(ri, text="",
                                font=("Helvetica", 8, "italic"),
                                fg=T["ok"], bg=T["surface"])
        self.sms_lbl.pack(anchor="w", pady=(2, 0))
        self._r(self.sms_lbl, "bg", "surface")
    ...
    # ── UI Helpers ─────────────────────────────────────────────
    def _r(self, w, opt, key):
        self._rw.append((w, opt, key))
    ...
    def _card(self, parent, expand=False):
        ...
        if expand:
            f.pack(fill="both", expand=True, pady=(0, 12))
        else:
    ...
    def _kv(self, parent, key, val="—", vcol="kv_val"):
        ...
        self._r(f, "bg", "surface")
        kl = tk.Label(f, text=key, font=("Helvetica", 9),
                      fg=T["kv_key"], bg=T["surface"])
        kl.pack(side="left")
        self._r(kl, "fg", "kv_key")
        self._r(kl, "bg", "surface")
        sv = tk.StringVar(value=val)
        vl = tk.Label(f, textvariable=sv,
                      font=("Helvetica", 10, "bold"),
                      fg=T[vcol], bg=T["surface"])
        vl.pack(side="right")
        self._r(vl, "fg", vcol)
        self._r(vl, "bg", "surface")
        return sv

    def _field_label(self, parent, text):
        T = self.T
        l = tk.Label(parent, text=text, font=("Helvetica", 9),
                     fg=T["sub"], bg=T["surface"])
        l.pack(anchor="w")
        self._r(l, "fg", "sub")
        self._r(l, "bg", "surface")
    ...
    def _dot(self, state):
        ...
        self.dot_cv.create_oval(1, 1, 9, 9,
                                fill=self.T[c.get(state, "sub")], outline="")

    # ── Classic Split Button ───────────────────────────────────
    def _draw_btn(self):
        c = self.btn_cv
        c.delete("all")
        T = self.T
    ...
    def _bhov_set(self, v):
        self._bhov = v
        self._draw_btn()

    def _on_btn(self, event):
        if not self._ben or self.mdl is None:
            return
        channel = (_TEST_FALL
                   if event.x < self.btn_cv.winfo_width() / 2
                   else _TEST_SAFE)
        threading.Thread(target=self._run_prediction_classic,
    ...
    def _run_prediction_classic(self, channel):
        ...
        self.root.after(0, self.st_lbl.configure, {"text": "Analysing Folder..."})
    ...
    def _on_sort_btn_click(self, event):
        if (self.busy or not self._ben
                or not self._live_buffer_ready
                or self.mdl is None):
            return
        _ch = (_TEST_FALL
               if event.x < (self.btn_sort_cv.winfo_width() / 2)
               else _TEST_SAFE)
        self.busy = True
        self._ben = False
        self._draw_btn()
        self._draw_sort_btn()
        threading.Thread(target=self._run_auto_sort_from_buffer,
                         args=(_ch,), daemon=True).start()
    ...
    def _serial_reader(self):
        # ── FIXED: use read(in_waiting) instead of readline()
        # This prevents USB driver overload, blue screen crashes,
        # and packet loss at 1,500,000 baud
        remainder = ""
        while True:
            try:
    ...
    def start_collection(self, label):
        if not self.ser:
            messagebox.showwarning("Hardware Offline",
                                   "Cannot record! ESP32 is not connected.")
            return
        if self.busy or self.is_recording:
            return
        self.busy = True
        self.data_buffer = []
        threading.Thread(target=self._run_sequence_manual,
                         args=(label,), daemon=True).start()

    def _run_sequence_manual(self, label):
        for i in range(5, 0, -1):
            self.root.after(0, self.lbl_timer.config,
                            {"text": f"Starting in {i}...", "fg": self.T["warn"]})
            time.sleep(1)
        try:
        ...
        while time.time() - start_t < SECS:
            ...
            self.root.after(0, self.lbl_timer.config,
                            {"text": f"REC: {remaining:.1f}s | Pkts: {len(self.data_buffer)}",
                             "fg": self.T["bad"]})
            time.sleep(0.1)

        self.is_recording = False
        self.root.after(0, self.lbl_timer.config,
                        {"text": "Saving Data...", "fg": self.T["ok"]})
            ...
            try:
                with open(path, "w", newline="") as f:
                    ...
                    for l in self.data_buffer:
                        w.writerow([l])
                self.root.after(0, self.lbl_timer.config,
                                {"text": f"Saved {len(self.data_buffer)} lines!"})
            except Exception:
                self.root.after(0, self.lbl_timer.config,
                                {"text": "Save Error!", "fg": self.T["bad"]})
        else:
            self.root.after(0, self.lbl_timer.config,
                            {"text": "Error: No Data Collected", "fg": self.T["bad"]})

        time.sleep(2)
        self.root.after(0, self.lbl_timer.config,
                        {"text": "Ready to Record", "fg": self.T["accent"]})
        self.busy = False
    ...
    def _on_live_record_click(self):
        ...
        self.root.after(0, self._draw_btn)
        self.root.after(0, self._draw_sort_btn)
        threading.Thread(target=self._run_live_record_and_sort,
                         daemon=True).start()

    def _run_live_record_and_sort(self):
        for i in range(5, 0, -1):
            self.root.after(0, self.lbl_live_status.config,
                            {"text": f"Recording in {i}…", "fg": self.T["warn"]})
            self.root.after(0, self.btn_live_record.config,
                            {"text": f"⏳  Starting in {i}s…", "bg": "#e8a020"})
            time.sleep(1)
        ...
        while time.time() - start_t < SECS:
            ...
            self.root.after(0, self.lbl_live_status.config,
                            {"text": f"⬤ REC  {remaining:.1f}s  |  {len(self.data_buffer)} pkts",
                             "fg": self.T["bad"]})
            self.root.after(0, self.btn_live_record.config,
                            {"text": f"⬤  RECORDING… {remaining:.1f}s", "bg": "#c0242f"})
            now = time.time()
            if now - last_plot_t >= 0.3 and len(self.data_buffer) > 10:
                last_plot_t = now
                raw_snap    = [l.split(",", 1)[-1] if "," in l else l
                               for l in list(self.data_buffer)]
                buf_snap    = parse_and_center_raw_lines(raw_snap)
                if buf_snap is not None and len(buf_snap) > 1:
                    sco = self.sc_var.get()
                    self.root.after(0, self._plot_live,
                                    buf_snap, max(0, min(sco, SC - 1)))
            time.sleep(0.1)

        self.is_recording = False
        self.root.after(0, self.btn_live_record.config,
                        {"text": "🔴  RECORD LIVE", "bg": "#f5505e"})
        ...
        self._live_raw_lines    = [l.split(",", 1)[-1] if "," in l else l
                                   for l in self.data_buffer]
    ...
    def _run_auto_sort_from_buffer(self, forced_channel=None):
        self.root.after(0, self.lbl_live_status.config,
                        {"text": "AI is Thinking…", "fg": self.T["accent"]})
        self.root.after(0, self.res_lbl.configure,
                        {"text": "Predicting…", "fg": self.T["sub"]})
        self.root.after(0, self.ico.configure, {"text": "·", "fg": self.T["sub"]})
        self.root.after(0, self.sms_lbl.configure, {"text": ""})

        if forced_channel:
            path = _acquire_test_filepath(forced_channel)
            if not path:
                self.root.after(0, self.lbl_live_status.config,
                                {"text": "AI Error: Data unreadable.", "fg": self.T["bad"]})
        ...
        # Always plot the live recorded buffer — not the file data
        live_buf = parse_and_center_raw_lines(self._live_raw_lines) \
                   if self._live_raw_lines else None

        buf  = parse_and_center_raw_lines(raw_lines)
        pred, cf, tag = _analyse(self.mdl, buf, self.scaler)
        sco  = self.sc_var.get()
        plot_buf = live_buf if live_buf is not None else buf
            ...
            with open(out_path, "w", newline="") as f:
                ...
                for l in raw_lines:
                    ...
                    if clean_l:
                        w.writerow([clean_l])
            self.root.after(0, self.lbl_live_status.config,
                            {"text": f"Live sorted & saved to '{label}'! ✔️",
                             "fg": self.T["ok"]})
        except Exception:
            self.root.after(0, self.lbl_live_status.config,
                            {"text": "Error saving file.", "fg": self.T["bad"]})

        self._live_buffer_ready = False
        self._live_raw_lines    = []
        self.root.after(0, self._on_result, pred, cf, tag, plot_buf, sco)
    ...
    def _run_auto_sort(self, mode):
        ...
        path = _acquire_test_filepath(
            _TEST_FALL if mode == "sim_fall" else _TEST_SAFE)
        if not path:
            self.root.after(0, self.lbl_live_status.config,
                            {"text": "Error: Could not find test file.", "fg": self.T["bad"]})
            self.busy = False; self._ben = True
            self.root.after(0, self._draw_btn)
            self.root.after(0, self._draw_sort_btn)
            return
            ...
            with open(out_path, "w", newline="") as f:
                ...
                for l in raw_lines:
                    ...
                    if clean_l:
                        w.writerow([clean_l])
            self.root.after(0, self.lbl_live_status.config,
                            {"text": f"Sorted & Saved to '{label}'! ✔️",
                             "fg": self.T["ok"]})
        except Exception:
            self.root.after(0, self.lbl_live_status.config,
                            {"text": "Error Saving File.", "fg": self.T["bad"]})
    ...
    def _on_result(self, pred, cf, tag, buf, sco):
        ...
        # WhatsApp alert on fall detection
        if pred == 1:
            self.sms_lbl.configure(
                text=f"📲 Sending WhatsApp alert to {ALERT_NUMBER}…",
                fg=T["warn"])
            def _send_and_update():
                send_fall_alert()
                self.root.after(0, self.sms_lbl.configure,
                                {"text": f"✅ WhatsApp sent to {ALERT_NUMBER}",
                                 "fg": T["ok"]})
            threading.Thread(target=_send_and_update, daemon=True).start()
        else:
            self.sms_lbl.configure(text="", fg=T["ok"])
    ...
    def _plot_live(self, buf, sc):
        ...
        self.ax.set_title(f"⬤ LIVE  Subcarrier #{sc}  ·  {len(amp)} packets",
                          color=T["bad"], fontsize=10, fontweight="bold")
        self.ax.set_xlabel("Time Step (Packets)", color=T["sub"], fontsize=9)
        self.ax.set_ylabel("EMA-Centered Magnitude", color=T["sub"], fontsize=9)
        self.fig.tight_layout(pad=1.8)
        self.pw.draw()
    ...
    def _plot(self, buf, sc, pred):
        ...
        self.ax.axvline(peak, color=col, linewidth=1.4,
                        linestyle="--", alpha=0.6, label="Max Disruption")
        self.ax.scatter([peak], [amp[peak]], color=col, s=30, zorder=5)
        self.ax.set_title(f"Processed Subcarrier #{sc}  ·  {len(amp)} packets",
                          color=T["text"], fontsize=10, fontweight="bold")
        self.ax.set_xlabel("Time Step (Packets)", color=T["sub"], fontsize=9)
        self.ax.set_ylabel("EMA-Centered Magnitude", color=T["sub"], fontsize=9)
        self.ax.legend(fontsize=8, facecolor=T["surface"],
                       edgecolor=T["border"], labelcolor=T["sub"])
        self.fig.tight_layout(pad=1.8)
        self.pw.draw()

    def _switch_theme(self):
        self.tn = "light" if self.tn == "dark" else "dark"
        self.T  = THEMES[self.tn]
        T       = self.T
        self.root.configure(bg=T["bg"])
        self.tgl.configure(text="☀" if self.tn == "dark" else "☾")
        for w, opt, key in self._rw:
            try:
                w.configure(**{opt: T[key]})
            except:
                pass
        self.sld.configure(bg=T["surface"], fg=T["sub"],
        ...
        for sp in self.ax.spines.values():
            sp.set_edgecolor(T["border"])
        self.ax.xaxis.label.set_color(T["sub"])
        self.ax.yaxis.label.set_color(T["sub"])
        self.ax.title.set_color(T["sub"])
    ...
    def _show_error_popup(self, title, message):
        ...
        lbl = tk.Label(err_win, text="The ResNet1D model failed to load.",
                       font=("Helvetica", 11, "bold"), fg="#f5505e")
        lbl.pack(pady=(10, 5))
        ...
        btn_copy = tk.Button(btn_frame, text="📋 Copy Error",
                             command=copy_err, font=("Helvetica", 9, "bold"))
        btn_copy.pack(side="left", padx=20)
        btn_ok = tk.Button(btn_frame, text="OK", command=err_win.destroy,
    ...
    def _init_model(self):
        def _go():
            try:
                ...
                if self.ser is None:
                    self.root.after(0, self.st_lbl.configure,
                                    {"text": "AI Ready  ·  Hardware Offline",
                                     "fg": self.T["warn"]})
                else:
                    self.root.after(0, self._dot, "ok")
                    self.root.after(0, self.st_lbl.configure,
                                    {"text": f"System Fully Active  ·  {_PORT}"})
            except Exception as e:
                self.root.after(0, self._dot, "err")
                self.root.after(0, self.st_lbl.configure,
                                {"text": "Model Error (See Popup)"})
                err_msg = f"Model: {_MP}\nScaler: {_SCALER}\n\n{str(e)}"
                self.root.after(0, lambda: self._show_error_popup(
                    "AI Load Error", err_msg))
